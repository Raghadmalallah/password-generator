...
function generateOptions() {
  ...
  var shouldIncludeUppercase = confirm(
    "Do you want to include uppercase characters?"
  );

  var shouldIncludeLowercase = confirm(
    "Do you want to include lowercase characters?"
  );

  var shouldIncludeNumeric = confirm(
    "Do you want to include numeric characters?"
  );

  var shouldIncludeSpecialCharacters = confirm(
...
function generatPassword() {
  ...
  for (var i = 0; i < option.length; ++i) {
    var randomPassword = Math.floor(
      Math.random() * Math.floor(passwordHub.length)
    );
